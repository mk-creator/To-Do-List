...
function plusFuncInitial(InputValue,code){
    ...
    li.setAttribute("onmousedown","MouseDown(this)");
...
function plusFunc(source){
    ...
    var itemsArray = JSON.parse(localStorage.getItem(code));   
    itemsArray.push(InputValue);
    // itemsArray.splice((itemsArray.length-1),0,InputValue)
    localStorage.setItem(code,JSON.stringify(itemsArray));
    var ul = document.getElementById("ul_"+code);
    var li = document.createElement("li");
    li.className = "listItem";
...
function MouseDown(source){
    ...
    source.addEventListener("mouseup", function(){handleMouseUp(source);});
    source.addEventListener("mousemove", function(){handleMouseMove(source);});
	    
}

function handleMouseMove(source) {
            ...
            if(mr<-500){
                ...
                tt = setInterval(function(){
                    ...
                    if(mr>15){
                        ...
                        new_element.setAttribute("onmousedown","MouseDown(this)");
                    }       
                },20);
            } 
